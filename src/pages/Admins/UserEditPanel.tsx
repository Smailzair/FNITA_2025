...
interface UserEditPanelProps {
    userId: string;
    onClose: () => void;
    onSave: () => void; // Callback to notify parent that data might have changed
}

export default function UserEditPanel({ userId, onClose, onSave }: UserEditPanelProps) {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            if (data) {
                ...
                // Check for pending email changes in the profile data if your trigger populates it
                // This part is speculative as I don't have your DB schema, but it's a safe alternative.
                // if (data.new_email) {
    ...
    async function HandleSubmit(
        ...
        setLoading(false);
        onSave(); // Notify parent that data might have changed
    }
    ...
    async function handleEmailUpdate() {
        ...
        // 1. Call the secure Edge Function to handle the email update
        const { error: functionError } = await supabase.functions.invoke('update-user-email', {
            body: { userId: userId, newEmail: formData.email },
        });

        if (functionError) {
            // The function might return a more specific error in its response body,
            // but for now, we'll show the generic invocation error.
            ...
            setError(`Erreur de mise à jour d'email: ${errorMessage}`);
            setEmailLoading(false);
            return;
        }
        ...
        // This admin call initiates the email change and sends a confirmation link to the new email.
        // The email is not updated until the user confirms.
        setEmailLoading(false);
        setEmailMessage("Un e-mail de confirmation a été envoyé à la nouvelle adresse de l'utilisateur.");
        setIsEmailEditing(false);
        // We don't update originalEmail here, as the change is not yet confirmed.
        // Instead, we can show the pending change.
        setIsEmailChangePending(true);
    ...
    async function handleAccountDelete(directDelete: boolean) {
        if (!userId) return;
        setLoading(true);
        setError("");
        setMessage("");
        ...
        if (directDelete) {
            ...
            try {
                ...
                setActDelMsg("Compte supprimé avec succès.");
                onSave(); // Notify parent to refresh list and close modal
            } catch (err: unknown) {
                const errorMessage = (err as Error)?.message;
                setError(`Erreur lors de la suppression du compte: ${errorMessage}`);
            } finally {
                setLoading(false);
            }
        } else {
            // This is the "request deletion" flow, toggling the flag
            const newStatus = !formData.asking_to_delete;
            const { error } = await supabase
                .from("tb_login")
                .update({ asking_to_delete: newStatus })
                .eq("id", userId);
            if (error) setError(error.message);
            else
                setActDelMsg(
                    `Demande de suppression de compte ${newStatus ? "envoyée" : "annulée"}.`
                );
            setFormData((p) => ({ ...p, asking_to_delete: newStatus }));
            onSave(); // Notify parent that data might have changed
        }
        ...
        if (loading) {
            return (
                <div className="flex justify-center items-center p-4">Chargement...</div>
            )
        }
        ...
        return (
            <div className="relative w-full">
                ...
                <div className="bg-stone-500 rounded-lg shadow-lg">
                    <form
                        className="p-4"
                    ...
                    >
                        {!isSmallScreen && (
                            <>
                                <h1 className="text-2xl font-bold text-slate-300 text-center items-center w-full pt-4">
                            ...
                            <div className="flex flex-col items-center justify-center w-full max-w-md px-4">
                                ...
                                {isEmailEditing && !isEmailChangePending && (
                                    <p className="text-yellow-300 text-xs w-full text-center mb-4">
                                        L'utilisateur devra confirmer le changement via sa nouvelle adresse e-mail.
                                    </p>
                                )}
                                {emailMessage && (
                            ...
                            {formData.asking_to_delete ? (
                                <>
                                    <p className="text-yellow-300 text-sm">
                                        L'utilisateur a demandé la suppression de son compte.
                                    </p>
                                    <button
                                        type="button"
                                        onClick={() => handleAccountDelete(true)} // Direct delete
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => handleAccountDelete(false)} // Cancel request
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleAccountDelete(false)} // Request deletion
