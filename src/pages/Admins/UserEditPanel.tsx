...
export default function UserEditPanel({
  ...
  async function handleEmailUpdate() {
    ...
    setEmailLoading(false);
    setEmailMessage(
      "Un e-mail de confirmation a été envoyé à la nouvelle adresse de l'utilisateur."
    );
    setIsEmailEditing(false);
    // We show the pending change, which will be fully applied after user confirmation.
    setIsEmailChangePending(true);
  ...
  async function handleCancelEmailChange() {
    ...
    const { error } = await supabase.rpc("cancel_email_change_for_user", {
      user_id: userId,
    });
  ...
  const handlePasswordReset = async () => {
    ...
    if (success) {
      setPasswordResetState({
        status: "sent",
        message:
          "Un lien de réinitialisation a été envoyé à l'e-mail de l'utilisateur.",
      });
    } else {
      ...
      <div className="bg-stone-500 rounded-lg shadow-lg">
        ...
        >
          ...
          <div className="row flex w-full justify-center items-center space-x-3">
            <div
              className={`flex flex-col items-center ${role !== "Administrateur" || originalType === "Vétérinaire" ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
              onClick={() =>
                role === "Administrateur" &&
          ...
          <div className="flex flex-wrap justify-center items-start">
            ...
            <label
              className={`flex ${formData.type === "Vétérinaire" ? "text-orange-200" : ""} w-72 items-center justify-end`}
            ...
            <div className="flex flex-col items-center justify-center w-full max-w-md px-4">
              {isEmailChangePending ? (
                <div className="text-center bg-yellow-200 text-yellow-800 p-3 rounded-lg w-full flex flex-col items-center">
                  ...
                  <p className="text-sm">
                    L'utilisateur doit confirmer sa nouvelle adresse e-mail :{" "}
                    <br />
              ...
              {isEmailEditing && !isEmailChangePending && (
                <p className="text-yellow-300 text-xs w-full text-center mb-4">
                  L'utilisateur devra confirmer le changement via sa nouvelle
                  adresse e-mail.
                </p>
              )}
              {emailMessage && (
